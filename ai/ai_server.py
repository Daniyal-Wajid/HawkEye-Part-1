...
def recognize_live():
    ...
    try:
        file = request.files.get("frame")
        if not file:
            print("[AI Server] ⚠ No frame received in request")
            return jsonify({"error": "No frame received"}), 400
        ...
        if frame is None:
            print("[AI Server] ❌ Failed to decode image frame")
        ...
        print(f"[AI Server] ✅ Analysis complete: {len(results)} faces found, {sum(1 for r in results if r['recognized'])} recognized")
