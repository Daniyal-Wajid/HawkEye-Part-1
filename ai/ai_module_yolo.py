...
DETECTION_CONFIDENCE = 0.30  # Lowered from 0.35
MIN_FACE_CONFIDENCE = 0.30   # Lowered from 0.45 for better live sensitivity
...
class FaceRecognizer:
    ...
    def recognize_all_faces(self, frame: np.ndarray) -> List[Dict]:
        ...
        try:
            # Detect faces once
            # Increase confidence slightly to avoid noise, but keep it lenient enough
            detections = self.detect_faces_yolo(frame, min_conf=0.30) 
            
            if not detections:
                if frame.shape[0] > 0 and frame.shape[1] > 0:
                    # Occasional log if truly no faces found in a valid frame
                    # print(f"[AI] No faces detected in frame {frame.shape}")
                    pass
                return []
            ...
            if not self.student_embeddings:
                print("[AI] No students registered in database. Returning detections only.")
                # Return detections marked as not recognized
                for d in detections:
                    x1, y1, x2, y2 = d[:4]
                    # Ensure bbox is [x, y, w, h] and integers
                    bbox_h = [int(x1), int(y1), int(max(0, x2-x1)), int(max(0, y2-y1))]
                    results.append({
                        "student_id": None,
                        "confidence": float(d[4]) if len(d) > 4 else 0.0,
                        "bbox": bbox_h,
                        "recognized": False
                    })
                return results
            ...
            for d in detections:
                ...
                if is_rec:
                    print(f"[AI] Recognized {best_match} ({best_similarity:.2f})")
            ...
            print(f"[AI] Critical Error in recognize_all_faces: {e}")
            import traceback
            traceback.print_exc()
            return []
